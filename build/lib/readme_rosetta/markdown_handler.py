...
class MarkdownHandler:
    ...
    def protect_code_blocks(self, text: str) -> Tuple[str, List[str]]:
        """
        Replaces code blocks with placeholders to prevent them from being translated.

        :param text: The Markdown text.
        :return: A tuple containing the protected text and a list of original code blocks.
        """
        placeholders = []
    ...
    def forge_stone(
        ...
        translated_lang_name = self.translator.translate(
            lang_name, start_code, end_code
        )
        new_header_link = (
            header_link + "-" + translated_lang_name.lower().replace(" ", "-")
        )
...
"""
    ...
    def translate_markdown(
        ...
        :return: A tuple containing (translated_content, rosetta_table, original_content_without_rosetta).
        """
        ...
        # Split into blocks (paragraphs) instead of lines for better context
        # We split by double newlines but preserve them
        blocks = re.split(r"(\n\s*\n)", protected_text)
        translated_blocks = []

        pbar = tqdm(
            total=len(
                [b for b in blocks if b.strip() and not re.search(r"ROSETTA_CB_\d+", b)]
            ),
        ...
        for block in blocks:
            # If block is empty, just whitespace, or just a placeholder, don't translate
            if not block.strip() or re.match(r"^ROSETTA_CB_\d+$", block.strip()):
                translated_blocks.append(block)
                continue

            # Check if block contains placeholders but also text
            # We still translate it but warn the LLM (done in Translator)
            translated_block = self.translator.translate(block, start_code, end_code)
            translated_blocks.append(translated_block)
            pbar.update(1)

        pbar.close()

        final_translated_text = "".join(translated_blocks)

        # Restore code blocks
        final_text = self.restore_code_blocks(final_translated_text, placeholders)

        rosetta_table = self.forge_stone(
            first_header, start_code, end_code, existing_table
        )

        return final_text, rosetta_table, source_without_rosetta
