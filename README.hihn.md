...
**README Rosetta** là một công cụ tự động hóa mạnh mẽ được thiết kế để dịch tài liệu của bạn thành nhiều ngôn ngữ khác nhau bằng cách sử dụng các mô hình LLM địa phương qua [Ollama](https://ollama.ai/). Nó đảm bảo rằng dự án của bạn có thể tiếp cận với khán giả toàn cầu trong khi duy trì định dạng Markdown hoàn hảo và cấu trúc tài liệu nguyên bản.
## 🌍 README Translation

README Rosetta specializes in making your GitHub project international with minimal effort.

- **Multi-language Support:** Translate `README.md` into dozens of languages simultaneously.
...
`$eFQ{The CLI is designed to be intuitive yet powerful.}$
### Installation
```bash
pip install readme-rosetta
```

*Note: Requirements [Ollama](https://ollama.ai/) must be installed and running on your system.*
### Global Options

| ہیں نمبر | وضاحت | اعداد و شمار |
| :--- | :--- | :--- |
| `path` | پتہ سوسائٹی یا پروجیکٹ ڈیریکٹری. | `README.md` |
| `--langs` | لیست ٹारگٹ لیanguage کوڈ (مثلا، `es fr de`). | `[]` |
| `--src-lang` | سوسائٹی زبان کوڈ. | `en` |
| `--model` | ہلاما ماڈل آئی اُں کا استعمال کرنا. | `llama3.2` |
| `--readme` | پتہ مین اوٹپوت ڈیریکٹری فیچر ریڈ میو. | `README.md` |
| `--no-split` | ترجمیشن کو ایک سولھی فائلا میں شامل کرنا. | `False` |
| `--dry-run` | پروسेस کو_simulate کرنا کے بغیر فائلز لکھنا. | `False` |
| `--verbose` | Detailed ہیٹنگ کا استعمال کریں گے اسٹیبلشمنٹ لی۔ | `False` |
## 📚 Hihn Integration

Hihn your documentation to professional levels with automated Hihn i18n support.

When you run with the ```bash
# Setup Sphinx with translations for Spanish and Japanese
readme-rosetta --sphinx --langs es ja
``` flag, README Hihn:
1.  **Initializes Hihn:** Sets up a ```bash
# Generate localized READMEs and a SUMMARY.md for GitBook
readme-rosetta --gitbook --langs hi zh pt
``` directory if it doesn't exist.
2.  **Auto-configures i18n:** Updates ```toml
[tool.readme-rosetta]
model = "llama3.2"
src-lang = "en"
langs = ["es", "fr", "de"]
...
sphinx = true
gitbook = false
``` with the necessary ```bash
# Run the cleanup utility
python3 scripts/cleanup_translations.py
``` and `README.md` settings.
3.  **Extracts Strings:** Runs `README.es.md` to find all translatable strings in your documentation.
4.  **Translates PO Files:** Uses the LLM to translate `README.fr.md` files, preserving Hihn-specific syntax like `--no-split` or `README.md`.
